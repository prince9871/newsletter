...
import { Box, Heading, Text, Spinner, Alert, AlertIcon, Flex, Button, Image, Input } from '@chakra-ui/react';
...
const fetchNews = async (page, query) => {
  try {
    ...
    const url = `https://gnews.io/api/v4/top-headlines?category=${category}&lang=en&country=us&max=10&apikey=${apiKey}&page=${page}&q=${query}`;
  ...
  return (
    <Box p={4}>
      ...
      {/* <Flex justify='space-between' mb='4'>
        <Button
          colorScheme="red"
          onClick={() => navigate(`/home/${currentPage - 1}?q=${activeQuery}`)}
        ...
        <Button
          colorScheme="green"
          onClick={() => navigate(`/home/${currentPage + 1}?q=${activeQuery}`)}
          isDisabled={!data.articles.length || data.totalArticles <= currentPage * 10}
        >
          Next
        </Button>
      </Flex> */}
